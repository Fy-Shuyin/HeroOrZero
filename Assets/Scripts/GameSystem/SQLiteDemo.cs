...
public class SQLiteDemo : MonoBehaviour 
{
	...
	void Start () 
	{
        ...
        //sql = new SQLiteHelper("data source=skilldate.db");

        //创建名为table1的数据表
        //sql.CreateTable("table1",new string[]{"ID","Name","ACT","CD","MP","EFFECT"},new string[]{"INTEGER","TEXT","INTEGER","INTEGER","INTEGER","INTEGER"});

        //插入两条数据
        //sql.InsertValues("table1",new string[]{"'1'","'FireBall'","'10'","'3'","'5'","'1'"});
        ...
        //读取整张表
        //SqliteDataReader reader = sql.ReadFullTable ("skill1");
        //Debug.Log (reader);
        //while(reader.Read()) 
        //{
        //读取ID
